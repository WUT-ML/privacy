...
class Generator(nn.Module):
    ...
    def __init__(self, image_size, conv_dim, in_dim):
        """Set parameters of generator neural network."""
        super(Generator, self).__init__()
        self.fc = deconv(in_dim, conv_dim*8, int(image_size/16), 1, 0, batch_norm=False)
    ...
    def forward(self, input, label=None):
        ...
        z = input.view(input.size(0), input.size(1), 1, 1)
        out = self.fc(z)
...
class Discriminator(nn.Module):
    ...
    def __init__(self, image_size, conv_dim, in_dim=1):
        """Set parameters of discriminator neural network."""
        super(Discriminator, self).__init__()
        self.conv1 = conv(3 * in_dim, conv_dim, 4, batch_norm=False)
    ...
    def forward(self, input, label=None):
        ...
        if label is not None:
            input = torch.cat([input, label], 1)
        out = F.leaky_relu(self.conv1(input), 0.05)
